...
st.markdown(
    """
    <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700;800;900&family=Cormorant+Garamond:ital,wght@0,300;0,400;0,500;1,300;1,400&family=Outfit:wght@300;400;500&display=swap" rel="stylesheet">
    """,
...
.tool-rag {
    background: rgba(77, 144, 255, 0.1);
    border: 1px solid rgba(77, 144, 255, 0.3);
    color: #6ea8ff;
}

.tool-data {
    background: rgba(77, 255, 144, 0.1);
    border: 1px solid rgba(77, 255, 144, 0.3);
    color: #7dffaf;
}

.tool-combined {
    background: rgba(255, 144, 77, 0.1);
    border: 1px solid rgba(255, 144, 77, 0.3);
    color: #ffb07a;
}
...
def get_url_display_name(url: str) -> str:
    ...
    if "open.canada.ca" in url:
        return "open.canada.ca — Government of Canada Open Data Portal"

    if "ircc.canada.ca" in url:
        parts = url.rstrip("/").split("/")
        filename = parts[-1] if parts else url
        filename = filename.replace("_", " ").replace("-", " ")
        if "." in filename:
            filename = filename.rsplit(".", 1)[0]
        return f"ircc.canada.ca — {filename[:60]}"

    if "canada.ca" in url:
        return "canada.ca — Government of Canada"

    try:
        parsed = urlparse(url)
        domain = parsed.netloc
        path = parsed.path.strip("/").split("/")[-1]
        path = path.replace("-", " ").replace("_", " ")[:40]
    ...
    with tabs[0]:
        if sources:
            for i, chunk in enumerate(sources[:3], 1):
                url = chunk.get("url", "")
                url_display = get_url_display_name(url)
                ...
                st.markdown(
                    ...
                    <div class="source-card">
                        ...
                        {f'<div class="url">🔗 {url_display}</div>' if url_display else ''}
                    </div>
                    """,
                    unsafe_allow_html=True
                )
...
with st.sidebar:
    ...
    suggested_questions = [
        ...
        "How does IRCC ensure fairness in automated permit decisions?",
        "What is an Algorithmic Impact Assessment?",
        "Does IRCC use artificial intelligence to make permit decisions?",
        "What is the PGWP program and how many holders are in Toronto?",
    ]
